...
from src.CUMCM2026_C.solve_problem4_2_forecast import run
def main():
 data=run(); out=paths.result_path('result4-2'); shutil.copy2(paths.RESULT_TEMPLATES['result4-2'],out); wb=openpyxl.load_workbook(out); wp,wc,we=wb.worksheets; by={r['date']:r for r in data['daily']}
 for r,(ds,rec) in enumerate(sorted(by.items()),2):
  d=date.fromisoformat(ds); wp.cell(r,1).value=datetime(d.year,d.month,d.day); p=np.asarray(rec['purchase_by_interval_kwh'])
 ...
 for r in range(2,wc.max_row+1):
  v=wc.cell(r,1).value; d=v.date() if isinstance(v,datetime) else v; key=d.isoformat() if isinstance(d,date) else str(d)[:10]
