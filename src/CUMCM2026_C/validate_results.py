...
* 校验所需的数据（电价、负载、光伏）来自 :mod:`src.CUMCM2026_C.io_attachments`，
  不重新解析原始 xlsx。
...
def check_plan_sheet(
    ...
    *,
    ...
    """校验 ``计划购电量`` 宽表（147 列 / 144 时段列 + 日期 + 2 汇总列）。
    ...
    if label_mode == "header":
        problems: list[str] = []
        if header != list(PLAN_HEADER):
            for i, (got, want) in enumerate(zip(header, PLAN_HEADER)):
                if got != want:
                    problems.append(f"列{i + 1}: 实测{got!r} 期望{want!r}")
            if len(header) != len(PLAN_HEADER):
                problems.append(f"表头列数 实测{len(header)} 期望{len(PLAN_HEADER)}")
        ok_head = not problems
        dates = [io.coerce_date(r[0]) for r in body]
        ok_dates = dates == PLANNED_RESULT_DATES
        detail = (
            f"表头 {'一致' if ok_head else '不一致: ' + '; '.join(problems[:5])}; "
        ...
        return CheckResult(
            criterion=f"{sheet}: 147 列表头 + 334 天(2025-02-01..12-31)",
            status="passed" if ok else "failed",
            detail=detail,
        )
...
def validate_planned_file(
    ...
    *,
    ...
    if filled:
        for day, charge, discharge, soc0, soc24 in filled[:5]:
            report.add(check_storage_recurrence(charge, discharge, soc0, soc24))
    else:
...
def validate_all(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    Args:
        repo_root: 仓库根目录；``None`` 表示使用 :mod:`src.config` 推导的默认值。
